...
@MainActor @Observable final class DiscoverViewModel {
    ...
    func load(providers: [Int], more: Bool = false) async {
        ...
        let requestedKey = "\(media.rawValue):\(mode.rawValue):\(providers.sorted())"
        let replacing = !more
        if replacing && requestedKey != contentKey { movies = []; page = 0; totalPages = 1 }
        contentKey = requestedKey
        if mode == .mine && providers.isEmpty { loading = false; return }
        defer { if generation == token { loading = false } }
        do {
            let response = try await repository.feed(media: media, mode: mode, providers: providers, page: replacing ? 1 : page + 1)
        ...
        } catch is CancellationError {} catch { if !Task.isCancelled && generation == token { self.error = error.localizedDescription } }
    }
}
