...
struct ProviderBadge: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            PosterImage(path: provider.logoPath, size: "w92").frame(width: 28, height: 28).clipShape(Rectangle())
        ...
        }.padding(7).background(CineTheme.surface, in: Rectangle())
            .overlay(Rectangle().strokeBorder(showsSubscription && storage.isSubscribed(provider) ? CineTheme.success.opacity(0.5) : CineTheme.border))
    ...
    var body: some View {
        ...
        Menu {
            ...
            HStack(spacing: 9) {
                ...
                .background(compact ? Color.black.opacity(0.74) : CineTheme.accent, in: Rectangle())
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(alignment: .top) {
                    ...
                    if let vote = movie.voteAverage, vote > 0 {
                        Label(vote.formatted(.number.precision(.fractionLength(1))), systemImage: "star.fill")
                            ...
                            .padding(.horizontal, 9).frame(minHeight: 32).background(.black.opacity(0.74), in: Rectangle())
                            .accessibilityLabel("TMDB puanı \(vote.formatted(.number.precision(.fractionLength(1)))) / 10")
                    }
                }.padding(8)
            }.clipShape(Rectangle())
                .overlay(Rectangle().strokeBorder(CineTheme.border)).contentShape(Rectangle())
            NavigationLink(value: movie) {
            ...
            if let availability = movie.providersTr, !availability.all.isEmpty {
                ...
                HStack(spacing: 5) {
                    ForEach(Array(providers.prefix(3))) { provider in
                        PosterImage(path: provider.logoPath, size: "w92").frame(width: 24, height: 24)
                            .clipShape(Rectangle())
                            .overlay(Rectangle().strokeBorder(availability.flatrate?.contains(where: { $0.id == provider.id }) == true && storage.isSubscribed(provider) ? CineTheme.success : .clear, lineWidth: 1.5))
                    }
                ...
                if providers.contains(where: { storage.isSubscribed($0) }), availability.flatrate?.isEmpty == false {
