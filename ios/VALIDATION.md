...
Apple uygulama kaydı oluşturuldu (`6814588419`), GitHub bağlantısı kuruldu. `CineSeeker TestFlight` iş akışı `feat/native-ios` değişikliklerinde çalışacak şekilde kaydedildi; zorunlu iOS 27 testi ve dahili TestFlight arşiv hazırlığı eklendi. Xcode 27 sabitlemesi ve TestFlight grup bağlantısının son düzenlemeleri sürüyor. TMDB Cloud secret onayı bekleniyor. İlk varsayılan `main` denemesi başarısız; bu dalda iOS projesi bulunmuyor. Başarılı Cloud arşivi ve TestFlight yüklemesi henüz doğrulanmadı.
