...
project = uid('project'); target = uid('app'); test_target = uid('test'); ui_target = uid('uitest')
...
ui_ref = add('uitest-product', fields(isa='PBXFileReference', explicitFileType='wrapper.cfbundle', path='CineSeekerUITests.xctest', sourceTree='BUILT_PRODUCTS_DIR'))
refs=[]; sources=[]; tests=[]; ui_tests=[]; resources=[]
for path in sorted((root/'CineSeeker').rglob('*.swift')) + sorted((root/'CineSeekerTests').rglob('*.swift')) + sorted((root/'CineSeekerUITests').rglob('*.swift')):
    ...
    (ui_tests if 'CineSeekerUITests/' in rel else tests if 'CineSeekerTests/' in rel else sources).append(build)
...
products=add('products', fields(isa='PBXGroup', children=arr([app_ref,test_ref,ui_ref]), name='Products', sourceTree=quote('<group>')))
...
ui_phases=[phase('ui-sources','PBXSourcesBuildPhase',ui_tests),phase('ui-frameworks','PBXFrameworksBuildPhase',[])]
...
ui_configs=configs('ui-',dict(PRODUCT_NAME=quote('$(TARGET_NAME)'),PRODUCT_BUNDLE_IDENTIFIER='com.projectcagla.cineseeker.uitests',GENERATE_INFOPLIST_FILE='YES',TEST_TARGET_NAME='CineSeeker',CODE_SIGN_STYLE='Automatic',TARGETED_DEVICE_FAMILY=quote('1,2')))
...
objects[ui_target]=fields(isa='PBXNativeTarget',buildConfigurationList=ui_configs,buildPhases=arr(ui_phases),buildRules='()',dependencies=arr([dep]),name='CineSeekerUITests',productName='CineSeekerUITests',productReference=ui_ref,productType=quote('com.apple.product-type.bundle.ui-testing'))
objects[project]=fields(isa='PBXProject',attributes=fields(BuildIndependentTargetsInParallel='YES',LastUpgradeCheck='2700'),buildConfigurationList=project_configs,compatibilityVersion=quote('Xcode 15.0'),developmentRegion='tr',hasScannedForEncodings='0',knownRegions=arr(['tr','en','Base']),mainGroup=main,productRefGroup=products,projectDirPath='""',projectRoot='""',targets=arr([target,test_target,ui_target]))
